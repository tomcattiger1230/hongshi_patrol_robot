...
NUC 的系统镜像默认已安装 ROS 2 Jazzy，并已具备 Fast DDS runtime、Python binding 和
`Robot320Dds` 类型，不在本项目部署流程中重复安装或编译通讯环境。首次构建：
...
`install/setup.bash`。`FASTDDS_SETUP` 只保留给非标准 NUC 镜像覆盖原生环境，正式 NUC
无需设置。
