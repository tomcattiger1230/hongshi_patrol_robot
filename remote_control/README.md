...
2026-09-18 联调：车上缺失的 `local_rtsp_proxy.py` 和 `ptz_agent.py` 已补齐。NUC
Wi-Fi 地址由现场路由器动态分配；Mac 使用 `hs-nuc14lnk.local` 自动解析当前地址，不再要求
手动修改私有 `.env`。另发现旧云端推流
...
`MvCameraControl`。当前工业相机模块已改为按设备序列号选择，并实现双路 NUC 本地录像、
远程启停、文件落盘和磁盘空间保护；未使用不适用于这些设备的 V4L2 录像命令。
...
| 4 | 两路海康 USB 录像 | 双路本地 MP4、远程参数/启停、1 Hz 确认及同网文件夹下载已实测 |

视频采用 RTSP 传输，不将高带宽视频塞进 JSON/DDS 状态消息。DDS 负责车辆指令、
遥测和任务状态；雷达点云单独限频降采样，地图需版本化。录像固定在 NUC 落盘，
Mac 只发送开始/停止任务并接收最高 1 Hz 的确认状态，避免 Wi-Fi 丢包破坏录像文件。
...
2026-09-23 已部署到当时的 DHCP 地址（当前地址可能变化）：`spatial_mapping.launch.py`、
...
2026-09-23 长时间建图检查时，Cartographer 仍以约 10 Hz 接收点云，但不断扩大的地图曾
超过桥接器 1,000,000 栅格的预览上限。桥接器现已改为自动提高预览分辨率并按块保守
降采样，保证输出不超过上限；每块保留最高占用概率，未知区域不会覆盖已知障碍。
...
2026-09-23 实车增加公网地图定位流程。“载入地图并自动定位…”会先在 Mac 预览所选
YAML/PGM，经现有 SSH Wi-Fi/SIM 选择器把规范化后的 `active.yaml + active.pgm` 上传到
NUC，然后重启独立 `robot320-spatial-localization.service` 以重新读取地图，并请求 AMCL
全地图粒子搜索和五次静止扫描更新。定位服务与 Cartographer 建图服务互斥，且不会启动
Nav2 或发送车辆运动指令。

自动搜索产生候选位姿不等于已经可靠收敛。GUI/后端必须继续依据 `/amcl_pose` 协方差判断；
在大面积重复结构中，静止的单次 360° 扫描可能存在多个相似解，需要恢复 B9 里程计后让
车辆低速走一小段弧线才能消歧。本次保存的是 YAML/PGM 栅格地图，不含 Cartographer
pose graph，因此不能使用 pose graph 的纯定位能力。

空间地图中可用鼠标左键按下选择机器人位置，并拖动给出车头方向；紫色圆点和箭头是待
发送的初始位姿。点击“发送手动初始位置”并确认后，GUI 通过安全通道发布 `/initialpose`，
随后自动请求五次无运动扫描更新。这是定位估计校正，不会发送底盘运动指令。应选择地图上
可辨认的粗略位置并保证箭头方向与真实车头一致，然后等待置信度变为“正在收敛”或“良好”。
...
完整 B9+IMU+EKF 自动导航与手动底盘控制尚未完成整合。

## 7. 两台海康 USB 工业相机：静态抓图与 NUC 本地录像
...
实测两路均返回 4096×2460 JPEG。曾出现同一相机冷启动首帧均值约 6.5/255、立即重试升到
105/255 的情况，证明黑图不是公网压缩造成，而是固定 12 帧预热不足。抓图脚本现会等待
连续自动曝光至少 5 秒后再编码；更新后相机 1 实测均值约 155/255。GUI 对低于 5/255 的
画面提示“接近全黑”，低于 25/255 提示“明显偏暗”，仍需结合现场镜头盖、光圈和照明判断。

相机不再依赖可能变化的 USB 枚举顺序，而是固定按序列号映射：`DB0168357` 为“右相机”，
`DB0168290` 为“左相机”。GUI 可选择左右同时录制或单独录制，设置 1/2/5/10 fps、
4096×2460 / 2048×1230 / 1280×768 / 1024×615 分辨率和 5–28800 秒时长，并发送
启动/停止命令。MVS 原始采集和 MP4 编码全部在 NUC 上进行，每次会话写入：

```text
/home/hs/haikang_records/YYYYMMDD_HHMMSS/
  metadata.json
  right.mp4 / left.mp4
  preview_right.jpg / preview_left.jpg
  right.log / left.log
```

录像本体不会通过公网或 GUI 通道上传。录制期间 GUI 最多每秒读取一次状态与低分辨率 JPEG
确认图；确认图不代表 MP4 的录制分辨率。NUC 可用空间低于 5 GiB 时拒绝新录像，已有录像
不会由 GUI 自动删除。

“检测同网并刷新记录”先将 NUC mDNS 地址解析为 IPv4，再确认 Mac 出口地址和 NUC 地址属于
同一个私有 `/24`，最后必须通过局域网 SSH 成功读取固定录像目录，才会列出录像并启用下载。
下载使用局域网 `scp` 将选中的整个日期时间文件夹保存到 Mac；检测失败时不会回退云端。
本次实机已验证双相机 1280×768、2 fps、5 秒录像，两路各 10 帧且 MP4 时长均为 5.0 秒。

页面提供“自动（局域网优先）/ 局域网 / 公网”选择。自动模式先直连
`ROBOT_LAN_SSH_HOST`，失败后经云端选择器回退；手选模式不跨链路回退，状态栏会标明本次
JPEG 的实际链路。公网转发默认使用随机空闲 loopback 端口，避免 GUI 异常退出留下旧 SSH
进程时固定端口冲突导致按钮永久不可用；隧道失败时还会自动重连。

`ROBOT_LAN_SSH_HOST` 默认是 NUC 的 mDNS 名称 `hs-nuc14lnk.local`，macOS 会将它解析到
路由器当前分配的地址（本次为 `192.168.88.254`）。因此 DHCP 地址再次变化时无需改 GUI。
若某个网络屏蔽 mDNS，可临时把该变量设为现场 IP，或直接选择“公网”。
本地 PTZ 代理会绕过 macOS 的全局 HTTP(S) 代理访问该 `.local` 地址，避免局域网请求被
上游代理错误返回 502；Bearer token 鉴权仍然保留。
PTZ 同时维护一条经云端 `12220` 选择器进入 NUC 的 SSH 转发：健康检查优先探测局域网，
局域网不可达时自动切换公网 Wi-Fi/SIM 链路。因此远端使用时视频和云台控制可以同时回退，
无需把车端 `8090` 端口暴露到公网。
...
- `scripts/robot320_onboard/hik_usb_record.py`
...
| 两台 USB 工业相机静态抓图/录像控制 | NUC→云双反向 SSH→选择器→Mac；MP4 留在 NUC | Wi-Fi/SIM 控制、双路本地录像已实测 |
| 工业相机录像文件下载 | 同一局域网内 Mac 直连 NUC `scp` | 已实现；禁止云端回退 |
| 雷达、地图、轨迹 | 降采样/压缩数据→双反向 SSH→Mac | Wi-Fi/SIM 均已实测 |
| 建图启停 | 同一 SSH 流的固定 start/stop 请求 | SIM 实测开始、地图回传和停止闭环 |
...
macOS/Windows 的“雷达与地图”页默认使用 WAN 模式，本地建立端口 `12225` 到云端选择器，
再通过 NUC 上的固定脚本订阅 ROS Domain 20。Linux 默认保留 DDS 模式；可用环境变量
`ROBOT_SPATIAL_TRANSPORT=wan|dds` 显式选择。WAN 模式只传最多 1200 点、约 5 Hz 的二维
预览、位姿、建图状态和压缩地图，不上传完整 MID-360 原始三维数据。地图超过 100 万栅格
时由 NUC 自动提高预览分辨率并保守降采样，因此仍可在 Mac 保存当前预览地图。
